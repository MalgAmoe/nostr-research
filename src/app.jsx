...
function HelpPage(props) {
  const workflow = [
    ["1", "Start broad", "Search a topic, person, note, or phrase on the relays. This gathers real Nostr events into a working corpus."],
    ["2", "Read the shape", "Use the facets and Map workspace to see recurring topics, accounts, event kinds, domains, dates, and relay coverage."],
    ["3", "Follow evidence", "Select a note and move into its conversation, references, author, mentions, reactions, shared topics, or the author’s network."],
    ["4", "Narrow or expand", "Choose whether the next operation replaces the corpus, adds to it, or keeps only matching events."],
    ["5", "Keep what matters", "Pin useful notes, save them as a collection, or save the query as a recipe that can be rerun later."],
  ];
  return <article class="overflow-hidden rounded border border-emerald-900 bg-emerald-950/10">
    <header class="border-b border-emerald-900 bg-[radial-gradient(circle_at_top_left,rgba(132,204,22,.12),transparent_55%)] p-6 sm:p-9">
      <div class="flex items-start gap-5"><div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-lime-500 font-mono text-2xl text-lime-300">?</div><div><div class="font-mono text-[10px] tracking-[.2em] text-emerald-600">FIELD MANUAL</div><h1 class="mt-2 text-2xl text-lime-100 sm:text-3xl">How to use Nostr Research</h1><p class="mt-3 max-w-3xl leading-7 text-emerald-400">This is an explorer for investigating Nostr, not a social feed. You gather a corpus of real events, inspect its structure, follow relationships, compare parts of it, and keep useful evidence for later.</p></div></div>
      <button onClick={props.onClose} class="mt-6 rounded border border-lime-700 px-4 py-2 font-mono text-xs text-lime-200 hover:bg-lime-300 hover:text-black">← BACK TO RESEARCH</button>
    </header>
    ...
    <section class="border-b border-emerald-900 p-6 sm:p-9"><HelpTitle index="04" title="Corpus controls"/><div class="mt-5 grid gap-4 md:grid-cols-3"><HelpCard title="REPLACE" accent="new direction"><p>The new result becomes the entire working corpus.</p></HelpCard><HelpCard title="ADD" accent="expand"><p>New results are merged into the current corpus without discarding what is already there.</p></HelpCard><HelpCard title="KEEP MATCHES" accent="intersect"><p>Retains only current events that also occur in the new result.</p></HelpCard></div><p class="mt-4 text-sm leading-6 text-emerald-600">Facets in the left sidebar filter what is visible without fetching anything. The composed-query fields for author, kind, tag, and date can retrieve from relays; domain and media are local refinements.</p></section>

    <section class="border-b border-emerald-900 p-6 sm:p-9"><HelpTitle index="05" title="Saving and returning"/><div class="mt-5 grid gap-4 md:grid-cols-3"><HelpCard title="COLLECTION" accent="evidence"><p>Pin individual notes with ◆, give the group a name, and save it as a durable evidence set.</p></HelpCard><HelpCard title="RECIPE" accent="method"><p>Saves the query and its current result IDs. Rerun it later to see what was added, missing, or unchanged.</p></HelpCard><HelpCard title="RETURN TO" accent="checkpoint"><p>Before a search or expansion changes the corpus, the app records a short-lived return point so you can undo that research turn.</p></HelpCard></div></section>

    <section class="p-6 sm:p-9"><HelpTitle index="06" title="Useful Nostr concepts"/><div class="mt-5 divide-y divide-emerald-950 rounded border border-emerald-950"><HelpTerm term="Event / note">A signed Nostr data object. Kind 1 is a short note, but profiles, follows, reactions, articles, and many other objects are events too.</HelpTerm><HelpTerm term="Account">A public key and the events signed by it. Names and profile pictures come from profile-metadata events and may be missing.</HelpTerm><HelpTerm term="Relay">A server that stores and serves events. Different relays contain different subsets of Nostr, so coverage is always partial.</HelpTerm><HelpTerm term="Tag / edge">Structured event metadata such as topics (t), accounts (p), event references (e or q), and addressable-event references (a).</HelpTerm><HelpTerm term="Corpus">The current set of events being researched. Views and facets are different ways of understanding that same set.</HelpTerm><HelpTerm term="Local archive">The IndexedDB cache in this browser. It contains real events previously retrieved from relays—not generated sample data.</HelpTerm></div><div class="mt-6 flex flex-wrap items-center gap-3"><button onClick={props.onClose} class="rounded bg-lime-300 px-5 py-2.5 font-mono text-xs font-bold text-black hover:bg-lime-200">START RESEARCHING →</button><span class="text-xs text-emerald-800">Your existing corpus remains intact while reading this page.</span></div></section>
  </article>;
}

function HelpTitle(props) { return <div class="flex items-center gap-3"><span class="font-mono text-[10px] text-emerald-700">[{props.index}]</span><h2 class="font-mono text-sm tracking-[.12em] text-lime-300">{props.title.toUpperCase()}</h2></div>; }
function HelpCard(props) { return <div class="rounded border border-emerald-900 bg-black/10 p-4"><div class="flex items-center justify-between gap-3"><h3 class="font-mono text-xs tracking-[.12em] text-lime-300">{props.title}</h3><span class="font-mono text-[9px] text-emerald-800">{props.accent}</span></div><div class="mt-3 space-y-2 text-sm leading-6 text-emerald-600 [&_li]:ml-4 [&_li]:list-disc [&_li]:marker:text-emerald-800">{props.children}</div></div>; }
function HelpTerm(props) { return <div class="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt class="font-mono text-xs text-emerald-300">{props.term}</dt><dd class="text-sm leading-6 text-emerald-600">{props.children}</dd></div>; }
...
function ComparisonLens(props) {
  ...
  const options = createMemo(() => dimension() === "account" ? ranked(props.events.map((event) => event.pubkey), 30) : dimension() === "topic" ? ranked(props.events.flatMap((event) => tags(event, "t").map((topic) => topic.toLowerCase())), 30) : ranked(props.events.flatMap((event) => sourceIndex.get(event.id) ?? []), 20));
  ...
  const eventsFor = (value) => !value ? [] : dimension() === "account" ? props.events.filter((event) => event.pubkey === value) : dimension() === "topic" ? props.events.filter((event) => tags(event, "t").some((topic) => topic.toLowerCase() === value)) : props.events.filter((event) => (sourceIndex.get(event.id) ?? []).includes(value));
