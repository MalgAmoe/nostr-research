...
While the local server is running, the client writes append-only JSON Lines telemetry to `logs/usage.ndjson`. It records queries, relay response status/timing, research actions, and saved-path summaries so we can see what the client is actually doing. This file remains in the reference-client directory; it is not sent to any third party beyond the selected public relays required for a query.

## Local development identity

`keys/dev-user.json` contains a generated, unencrypted development keypair in hexadecimal and NIP-19 (`nsec` and `npub`) forms. Run `npm run keys:generate` from the repository root or client directory to replace it. The local server blocks HTTP access to both `keys/` and `logs/`; treat the identity as disposable and never fund it.
