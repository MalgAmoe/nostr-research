...
function App() {
  ...
  async function ensurePulseRestored() {
    if (pulseRestorePromise) return pulseRestorePromise;
    pulseRestorePromise = (async () => {
      const started = performance.now();
      const pulseRestored = load(PULSE_SESSION_KEY, null);
      if (pulseRestored?.eventIds?.length) {
        const [current, previous] = await Promise.all([
          loadEvents(pulseRestored.eventIds, runtime.recordSources),
          loadEvents(pulseRestored.previousIds ?? [], runtime.recordSources),
        ]);
        relayExplorer.restore({
          events: allowedEvents(current),
          previous: allowedEvents(previous),
          meta: pulseRestored.meta ?? null,
          round: pulseRestored.round ?? 0,
          reasons: pulseRestored.reasons ?? {},
        });
      }
      setPulsePersistenceReady(true);
      logUsage("relay_explorer_restored", {
        events: pulseEvents().length,
        durationMs: Math.round(performance.now() - started),
      });
    })();
    return pulseRestorePromise;
  }

  async function loadRoute(next = parseRoute()) {
    if (next.kind === "search" || next.kind === "relays" || next.kind === "settings") {
      setRoute(next); setRouteLoading(false); setRouteData(null);
      if (next.kind === "relays") void ensurePulseRestored();
      return;
    }
  ...
  onMount(async () => {
    ...
    if (route().kind === "relays") await ensurePulseRestored();
    logUsage("startup_restored", {
      route: route().kind,
      corpus: corpus().length,
      pulseLoaded: Boolean(pulseRestorePromise),
...
function ResearchTable(props) {
  ...
  const [shown, setShown] = createSignal(200);
  ...
  return <div><LensHeader title="RESEARCH TABLE" detail={`${Math.min(shown(), rows().length)}/${rows().length} rows shown · ${props.events.length} visible events`}/><div class="border-b border-emerald-950 p-3"><input aria-label="Search within current corpus" value={localQuery()} onInput={(event) => { setLocalQuery(event.currentTarget.value); setShown(200); }} placeholder="Search inside this corpus: text, account, topic, domain, or kind…" class="w-full rounded border border-emerald-900 bg-black/30 px-3 py-2 font-mono text-xs text-emerald-200 outline-none placeholder:text-emerald-900 focus:border-lime-700"/></div><Show when={rows().length} fallback={<EmptyLens text="No events match this local corpus search."/>}>
...
function SearchView(props) {
  const [shown, setShown] = createSignal(80);
  createEffect(() => { props.query; setShown(80); });
  return <section>
    <Show when={props.events.length} fallback={<div class="p-8 text-sm text-emerald-800">{props.query ? (props.loading ? "Waiting for the first relay response…" : "No events returned.") : "No default feed. Start with a question, account, note, or topic."}</div>}>
      <For each={props.events.slice(0, shown())}>{(event, index) => <EventRow event={event} index={index() + 1} profile={props.profileFor(event.pubkey)} pinned={props.pinned.has(event.id)} selected={props.selectedId === event.id} openRoute={props.openRoute} onSelect={props.onSelect} onNavigate={props.onNavigate} onPin={props.onPin} actions={props.actionsFor("event", event.id, short(event.id), { filter: false, compare: false })}/>}</For>
      <Show when={shown() < props.events.length}><div class="border-t border-emerald-950 p-4 text-center"><button onClick={() => setShown((count) => count + 80)} class="rounded border border-emerald-800 px-5 py-2 font-mono text-[10px] text-emerald-400 hover:text-lime-300">SHOW 80 MORE RETRIEVED NOTES · {props.events.length - shown()} HIDDEN</button></div></Show>
    </Show>
  </section>;
}
