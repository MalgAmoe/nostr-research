...
function useResearchBusy() {
  const legacyBusy = useLiveStore((state) => state.phase.type === 'working');
  const navigatorBusy = useAtlasStore((state) => Object.values(state.navigatorOperations).some((operation) => operation.status === 'working'));
  return legacyBusy || navigatorBusy;
}
...
function AuthorResolutionPanel({ placeId }: { placeId: string }) {
  ...
  return <section className="author-resolution-panel is-open">
    ...
    <button className="external-action" disabled={!draft.relays.length || busy} onClick={() => execute(placeId)}>Execute bounded author resolution</button>
...
function RelationshipDraft({ placeId, noteId, draft }: { placeId: string; noteId: string; draft: RelationshipActionDraft }) {
  ...
  return <section className="relationship-draft">
    ...
    <label className="draft-warning"><input type="checkbox" checked={draft.excludeContentWarnings} onChange={(event) => patch({ excludeContentWarnings: event.target.checked })}/> Preserve configured direct-warning exclusion</label>
    <button className="external-action" disabled={!draft.relays.length || busy} onClick={() => execute(placeId, noteId)}>Execute relay continuation and open branch</button>
...
function NoteDoors({ note, placeId, observation }: { note: Note; placeId: string; observation: NoteObservation }) {
  ...
  return <section className="note-doors">
    ...
    <div className="relationship-routes">{NOTE_RELATIONSHIPS.map((route) => {
      ...
      return <article key={route.value}><strong>{route.label}</strong><code>continue · {route.value} · source local · eventLimit {research?.relationshipDraft.eventLimit ?? 20}</code><div><button disabled={!observation.eventHandleId || busy} onClick={() => openLocal(placeId, note.id, route.value)}>Open bounded local branch</button><button disabled={!observation.eventHandleId} onClick={() => prepare(placeId, note.id, route.value)}>Prepare relay draft</button></div>{(['local', 'relays'] as const).map((source) => { const attempt = attempts?.[source]; return attempt ? <small key={source} className={`attempt-${attempt.status}`}>{source.toUpperCase()} · {attempt.status.toUpperCase()} · {attempt.count ?? 0} events · handle {attempt.handleId ?? 'unavailable'}</small> : null; })}</article>;
    })}</div>
